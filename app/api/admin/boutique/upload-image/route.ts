...
export async function POST(request: Request) {
  ...
  try {
    ...
    const fileName = `boutique-${Date.now()}-${randomUUID().slice(0, 8)}.${ext}`;
    const uploadsDir = path.join(process.cwd(), "public", "uploads", "boutique");
    ...
    console.error("Erreur POST /api/admin/boutique/upload-image:", error);
